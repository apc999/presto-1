        ...
        extends ExtendedFileSystem
{
    ...
    public FSDataInputStream open(Path path, int bufferSize)
            throws IOException
    {
        return new BasicCachingInputStream(dataTier.open(path, bufferSize), cacheManager, path, cacheValidationEnabled);
    }
            ...
            throws Exception
    {
        if (hiveFileContext.isCacheable()) {
            return new BasicCachingInputStream(dataTier.openFile(path, hiveFileContext), cacheManager, path, cacheValidationEnabled);
        }
